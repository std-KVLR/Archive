...
{
    if(color().visibility())
        fl_arc(point(0).x, point(0).y, w+w, h+h, startA, endA);
}
...
//------------------------------------------------------------------------------
 ...
 Smiley::Smiley(Point p, int rr) : r(rr)
{
    add(Point(p.x-r,p.y-r));
    add(Point(p.x - (r*0.6 + r*0.1), p.y-r*0.6));
    add(Point(p.x + r*0.1, p.y-r*0.6));
    add(Point(p.x - r*0.8, p.y - r*0.8));
}
...
void Smiley::draw_lines() const
{
    if (color().visibility()){
        ...
        fl_arc(point(1).x, point(1).y, r/3 + r/3, r/3 + r/3, 0, 360);
                                                                      //   Eyes
        fl_arc(point(2).x, point(2).y, r/3 + r/3, r/3 + r/3, 0, 360);

        fl_arc(point(3).x, point(3).y, r * 0.8 + r * 0.8 , r * 0.8 + r * 0.8, 195, 345); // Smile
    }
}
...
//------------------------------------------------------------------------------
 ...
 Frowney::Frowney(Point p, int rr) : r(rr)
{
    add(Point(p.x-r,p.y-r));
    add(Point(p.x - (r*0.6 + r*0.1), p.y-r*0.6));
    add(Point(p.x + r*0.1, p.y-r*0.6));
    add(Point(p.x - r*0.8, p.y + r*0.1));
}
...
void Frowney::draw_lines() const
{
    if (color().visibility()){
        ...
        fl_arc(point(3).x, point(3).y, r * 0.8 + r * 0.8 , r * 0.8 + r * 0.8, 20, 160); // Smile
    }
}
...
StripedRectangle::StripedRectangle(Point xy, int ww, int hh)
        : fp(xy), w(ww), h(hh)
{
    ...
    if(w == h) striping(fp, w, h, true);
    else striping(fp, w, h, false);
}
...
void StripedRectangle::draw_lines() const
{
    if (color().visibility()) {    // lines on top of fill
        ...
        for(size_t i = 1; i < number_of_points() - 2; i+=2){
                cout << "A :" << point(i).x << ';' << point(i).y << ", "
                     << "B :" << point(i + 1).x << ';' << point(i + 1).y << '\n';
                }
        for(size_t i = 1; i < number_of_points() - 2; i+=2){
...
void StripedRectangle::striping(const Point& fp, int w, int h, bool eq)
{
    const int step = 2;
    int lowest, higest;

    if(w < h) lowest = w, higest = h;
    else lowest = h, higest = w;

    size_t i = 1;
    for( ; i <= lowest; i+=step){
        add(Point(fp.x + i,fp.y));
        add(Point(fp.x ,fp.y + i));
    }
    add(Point((fp.x + w) - i , fp.y + h));
    add(Point(fp.x + w, (fp.y + h) - i));
    if(!eq){
        if(lowest == w){
            for(size_t j = 1; i < higest; i +=step, j+=step){
                add(Point(fp.x , fp.y + i));
                add(Point(fp.x + w, fp.y + j));
            }
        }
        else{
            for(size_t j = 1; i < higest; i +=step, j+=step){
                add(Point(fp.x + i , fp.y));
                add(Point(fp.x + j, fp.y + h));
            }
        }
    }

    for(i = 2; i <= lowest; i+=step){
        add(Point((fp.x + w) - i , fp.y + h));
        add(Point(fp.x + w, (fp.y + h) - i));
    }
    add(Point((fp.x + w) - i , fp.y + h));
