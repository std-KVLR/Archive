...
protected:
    ...
    void erase_points() { points.erase(points.begin(), points.end());}
private:
...
struct Func : Shape {
    ...
    void changeFunc(Fct f)
     {
        funct = f;
        redefine(funct, range1, range2, center, countNum, xScale, yScale);
     }
    void changeR1R2(double r1, double r2)
    {
        range1 = r1, range2 = r2;
        redefine(funct, range1, range2, center, countNum, xScale, yScale);
    }
    void changeCenter(Point centerV)
    {
        center = centerV;
        redefine(funct, range1, range2, center, countNum, xScale, yScale);
    }
    void changeCount(int newC)
    {
        countNum = newC;
        redefine(funct, range1, range2, center, countNum, xScale, yScale);
    }
    void changeScaling(double xscale, double yscale)
    {
        xScale = xscale, yScale = yscale;
        redefine(funct, range1, range2, center, countNum, xScale, yScale);
    }
    Func(Fct f, double r1, double r2, Point xy,
        int count = 100, double xscale = 25, double yscale = 25)
        : funct(f), range1(r1), range2(r2), center(xy),
          countNum(count), xScale(xscale), yScale(yscale)
        {
            redefine(funct, range1, range2, center, countNum, xScale, yScale);
        }
...
private:
    ...
    void redefine(Fct f, double r1, double r2, Point xy,
                int count = 100, double xscale = 25, double yscale = 25)
                {
                    erase_points();
